  ...
  | `Ret ->
      Logs.debug (fun m ->
          m "ptph_h Ret [pt: %a, ph: %a]" Sexp.pp_hum (Path.sexp_of_t pt)
            Sexp.pp_hum (T.sexp_of_phase ph))
  | `Rd_pt ->
      Logs.debug (fun m ->
  ...
  | `Update_st (path, label, (v, q)) ->
      Logs.debug (fun m ->
          m "mem_h Update_st [mem: %a, path: %a, label: %a, v: %a, q: %a]"
            ...
            Sexp.pp_hum (T.sexp_of_value v) Sexp.pp_hum (Job_q.sexp_of_t q))
  | `Get_dec path ->
      Logs.debug (fun m ->
  ...
  | `Set_dec (path, dec) ->
      Logs.debug (fun m ->
          m "mem_h Set_dec [mem: %a, path: %a, dec: %a]" Sexp.pp_hum
            (Tree_mem.sexp_of_t mem) Sexp.pp_hum (Path.sexp_of_t path)
            Sexp.pp_hum (T.sexp_of_decision dec))
  | `Enq_eff (path, clos) ->
      Logs.debug (fun m ->
          m "mem_h Enq_eff [mem: %a, path: %a, clos: %a]" Sexp.pp_hum
            (Tree_mem.sexp_of_t mem) Sexp.pp_hum (Path.sexp_of_t path)
            Sexp.pp_hum (T.sexp_of_clos clos))
  | `Alloc_pt ->
      Logs.debug (fun m ->
  ...
  | `Update_ent (path, ent) ->
      Logs.debug (fun m ->
          m "mem_h Update_ent [mem: %a, path: %a, ent: %a]" Sexp.pp_hum
            (Tree_mem.sexp_of_t mem) Sexp.pp_hum (Path.sexp_of_t path)
            Sexp.pp_hum (T.sexp_of_entry ent))
...
let render path vss =
  Logs.debug (fun m ->
      m "render [path: %a, vss: %a]" Sexp.pp (Path.sexp_of_t path) Sexp.pp
        (List.sexp_of_t T.sexp_of_view_spec vss))

let render1 vs =
  Logs.debug (fun m -> m "render1 [vs: %a]" Sexp.pp (T.sexp_of_view_spec vs))

let update path =
  Logs.debug (fun m -> m "update [path: %a]" Sexp.pp (Path.sexp_of_t path))

let update1 t =
  Logs.debug (fun m -> m "update1 [t: %a]" Sexp.pp (T.sexp_of_tree t))

let reconcile path old_trees vss =
  Logs.debug (fun m ->
      m "reconcile [path: %a, old_trees: %a, vss: %a]" Sexp.pp
        (Path.sexp_of_t path) Sexp.pp
        (List.sexp_of_t (Option.sexp_of_t T.sexp_of_tree) old_trees)
        Sexp.pp
        (List.sexp_of_t T.sexp_of_view_spec vss))
...
let reconcile1 old_tree vs =
  Logs.debug (fun m ->
      m "reconcile1 [old_tree: %a, vs: %a]" Sexp.pp
        ((Option.sexp_of_t T.sexp_of_tree) old_tree)
        Sexp.pp (T.sexp_of_view_spec vs))

let commit_effs path =
  Logs.debug (fun m -> m "commit_effs [path: %a]" Sexp.pp (Path.sexp_of_t path))

let commit_effs1 t =
  Logs.debug (fun m -> m "commit_effs1 [t: %a]" Sexp.pp (T.sexp_of_tree t))
