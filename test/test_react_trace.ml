...
(*   open Syntax.Expr *)
(**)
(*   let mkexp e = mk ~loc:Location.none e *)
(*   let mkconst c = mkexp (Const c) *)
(*   let mkvar x = mkexp (Var x) *)
(*   let mkview es = mkexp (View es) *)
(*   let mkcond ~pred ~con ~alt = mkexp (Cond { pred; con; alt }) *)
(*   let mkfn ~param ~body = mkexp (Fn { param; body }) *)
(*   let mkapp ~fn ~arg = mkexp (App { fn; arg }) *)
(*   let mklet ~id ~bound ~body = mkexp (Let { id; bound; body }) *)
(**)
(*   let mkstt ~label ~stt ~set ~init ~body = *)
(*     mkexp (Stt { label; stt; set; init; body }) *)
(**)
(*   let mkeff e = mkexp (Eff e) *)
(*   let mkseq (e1, e2) = mkexp (Seq (e1, e2)) *)
(*   let mkbop ~op ~left ~right = mkexp (Bop { op; left; right }) *)
(*   let mkuop ~op ~arg = mkexp (Uop { op; arg }) *)
(*   let mkalloc = mkexp Alloc *)
(*   let mkset ~obj ~idx ~value = mkexp (Set { obj; idx; value }) *)
(*   let mkget ~obj ~idx = mkexp (Get { obj; idx }) *)
(* end *)
(**)
(* let parse_prog s = *)
...
(*     type a. *)
(* (string -> string) -> a Syntax.Expr.t -> a Syntax.Expr.t -> a
   Syntax.Expr.t *)
(*     = *)
(*   let open Syntax.Expr in *)
...
(* {| *) (* a || b; a && b; a ?? b; *) (* a === b; a !== b; a < b; a <= b; a >
   b; a >= b; *) (* a + b; a - b; a * b; *) (* void a; -a; +a; !a|} *)
(*   in *)
(*   let prog = Js_syntax.convert js in *)
(*   Alcotest.(check' (of_pp Sexp.pp_hum)) *)
(*     ~msg:"parse obj" ~actual:(Prog.sexp_of_t prog) *)
(*     ~expected: *)
(*       (parse_prog *)
(* {| *) (* (let a' = a in if a' then a' else b); *) (* (let a'' = a in if a''
   then b else a''); *) (* (let a''' = a in if a''' = () then b else a'''); *)
...
(*   Alcotest.(check' (of_pp Sexp.pp_hum)) *)
(*     ~msg:"parse obj" ~actual:(Prog.sexp_of_t prog) *)
(* ~expected:(parse_prog "if a then (b; ()) else (c; ()); ()" |>
   Prog.sexp_of_t) *)
(**)
(* let js_pattern_id () = *)
(*   let open Syntax in *)
...
(*     ~msg:"parse obj" ~actual:(Prog.sexp_of_t prog) *)
(*     ~expected: *)
(* (parse_prog {| *) (* let q' = q in *) (* let x = q'["x"] in *) (* let y =
   q'["y"] in *) (* ()|} *)
(*       |> alpha_conv_prog Fun.id prog *)
(*       |> Prog.sexp_of_t) *)
(**)
...
(*     ~expected: *)
(*       (parse_prog *)
(* {| *) (* let q' = q in *) (* let x = q'[0] in *) (* let y = q'[1] in *) (*
   let z = q'[3] in *) (* ()|} *)
(*       |> alpha_conv_prog Fun.id prog *)
(*       |> Prog.sexp_of_t) *)
(**)
...
(*     ~expected: *)
(*       (parse_prog *)
(* {| *) (* let q' = q in *) (* let x = q'["x"] in *) (* let y = x["y"] in *) (*
   let a = y[0] in *) (* let b = y[1] in *) (* ()|} *)
(*       |> alpha_conv_prog Fun.id prog *)
(*       |> Prog.sexp_of_t) *)
(**)
(* let js_object () = *)
...
(*     ~expected: *)
(*       (parse_prog *)
(* {| *) (* let p = (let obj = {} in obj["y"] := 1; obj["z"] := 2; obj[3] := 4;
   obj) in p["y"]; p[1+2]; ()|} *)
(*       |> alpha_conv_prog Fun.id prog *)
(*       |> Prog.sexp_of_t) *)
(**)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* view [()] *) (*
   ;; *) (* view [C ()] *) (* |} *)
(*   in *)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* setS (fun s ->
   43); *) (* view [()] *) (* ;; *) (* view [C ()] *) (* |} *)
(*   in *)
(*   let run () = *)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* if s = 42 then
   setS (fun s -> 43); *) (* view [()] *) (* ;; *) (* view [C ()] *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step one time" ~expected:1 ~actual:steps *)
(**)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (setS
   (fun s -> 42)); *) (* view [()] *) (* ;; *) (* view [C ()] *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (if s
   = 42 then setS (fun s -> 43)); *) (* view [()] *) (* ;; *) (* view [C ()] *)
   (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step two times" ~expected:2 ~actual:steps *)
...
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (if s
   <= 45 then setS (fun s -> s + 1)); *) (* view [()] *) (* ;; *) (* view [C ()]
   *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
...
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (if s
   <> x then setS (fun s -> x)); *) (* view [()] *) (* ;; *) (* view [C 42] *)
   (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step one time" ~expected:1 ~actual:steps *)
...
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (if s
   <> x then setS (fun s -> x)); *) (* view [()] *) (* ;; *) (* view [C 0] *) (*
   |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step two times" ~expected:2 ~actual:steps *)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C setS = *) (* useEffect (setS (fun s -> 0)); *) (* view [()] *)
   (* ;; *) (* let D x = *) (* let (s, setS) = useState 42 in *) (* view [C
   setS] *) (* ;; *) (* view [D ()] *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step two times" ~expected:2 ~actual:steps *)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C setS = *) (* useEffect (setS (fun s -> s + 1)); *) (* view
   [()] *) (* ;; *) (* let D x = *) (* let (s, setS) = useState 42 in *) (* view
   [C setS] *) (* ;; *) (* view [D ()] *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(* Alcotest.(check' int) ~msg:"step indefintely" ~expected:fuel ~actual:steps *)
...
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (setS
   (fun s -> 43); setS (fun s -> 42)); *) (* view [()] *) (* ;; *) (* view [C
   ()] *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step one time" ~expected:1 ~actual:steps *)
...
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (setS
   (fun s -> s + 1)); *) (* view [()] *) (* ;; *) (* let D x = *) (* let (s,
   setS) = useState true in *) (* useEffect (setS (fun s -> false)); *) (* if s
   then *) (* view [C ()] *) (* else *) (* view [()] *) (* ;; *) (* view [D ()]
   *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step two times" ~expected:2 ~actual:steps *)
...
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState 42 in *) (* useEffect (setS
   (fun s -> 0)); *) (* view [()] *) (* ;; *) (* let D x = *) (* let (s, setS) =
   useState true in *) (* useEffect (setS (fun s -> false)); *) (* if s then *)
   (* view [C ()] *) (* else *) (* view [C ()] *) (* ;; *) (* view [D ()] *) (*
   |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step two times" ~expected:2 ~actual:steps *)
...
(*   let prog = *)
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState (let r = {} in r["x"] :=
   42; r) in *) (* useEffect (if s["x"] <= 45 then setS (fun s -> (let r = {} in
   r["x"] := s["x"] + 1; r))); *) (* view [()] *) (* ;; *) (* view [C ()] *) (*
   |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step five times" ~expected:5 ~actual:steps *)
...
(*     parse_prog *)
(* {| *) (* let C x = *) (* let (s, setS) = useState (let r = {} in r["x"] :=
   42; r) in *) (* useEffect (s["x"] := 43); *) (* view [()] *) (* ;; *) (* view
   [C ()] *) (* |} *)
(*   in *)
(*   let { Interp.steps; _ } = Interp.run ~fuel prog in *)
(*   Alcotest.(check' int) ~msg:"step one time" ~expected:1 ~actual:steps *)
